...
int main() {
	...
	auto mat = MaterialPhong(REDPINK_PALLETTE[1], .037, .41556, 0.16248, 52.0);
	auto floormat = MaterialPhong(BLUE_PALLETTE[5], .05, .41536, .0112348, 1.0);

	auto fn = loadSequence(CodeFileDescriptor("duel.txt", "sounds", true), vec2(0, 254)).downsample_max(4);

	cout << "Size: " << fn.samples() << endl;
	cout << "Sampling : " << fn.samples()/254.f << "Hz" << endl;

	int window = 512;
	int step = 128;

	cout << "Window: " << 254.f/fn.samples()*window << "s" << endl;
	cout << "Step: " << 254.f/fn.samples()*step << "s" << endl;
