...
ivec2 getResolutionSize(Resolution resolution) {
	unordered_map<Resolution, ivec2> resolutionToSize = {
		{FHD, ivec2(1920, 1080)},
		{HD2K, ivec2(2560, 1440)},
		{UHD, ivec2(3840, 2160)}
	};
	if (resolutionToSize.contains(resolution))
		return resolutionToSize[resolution];
	if (resolution == UNKNOWN)
		throw ValueError("Cannot get size of UNKNOWN resolution", __FILE__, __LINE__);
	throw ValueError("Unknown resolution enum value", __FILE__, __LINE__);
}
