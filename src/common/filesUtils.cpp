...
Path ConfigFile::getMainShaderDirectory() const { return Path(check("templateShaders")); }
...
ShaderMethodTemplateFromUniform ShaderMethodTemplateFromUniform::composeReturnWith(const ShaderMethodTemplate &f2) const {
	return ShaderMethodTemplateFromUniform(f2.getReturnType(), getName(), getArguments(), getBody(),
										   f2.generateCall(getReturnCode()), keys, _firstIndex, arrayName); }
