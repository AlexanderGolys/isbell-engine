...
float Complex::arg() const { return atan2(z.y, z.x); }
...
bool Complex::nearlyEqual(Complex c) const { return abs(norm(z - c.z)) < 1e-6; }
...
Complex exp(Complex c) { return Complex(cos(c.im()), sin(c.im())) * std::exp(c.im()); }
