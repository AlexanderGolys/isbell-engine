...
int main() {
	...
	SDFMaterialPlus torMat = SDFMaterialPlus(s2Diffuse, s2Diffuse, 0, .7, 1.33, 3.1);
...
//	auto box1 = roundBoxSDF(boxMat, vec3(.15, 0.7, 2.3), .15, vec3(-1, 0, 1), rotationMatrix(vec3(-2.3, -2.2, 0), 1.4));
	auto tor = torusSDF(torMat, .3, 1.5, vec3(1, -1, 1)).smoothUnion(sphereSDF(torMat, 1, vec3(0, -1, 1)), .5);
	auto floor = planeSDF(floorMat, vec3(0, 0, 1),1);
//		auto tor = torusSDF(torMat, .5, 1.5, vec3(1, 0, .5), rotationMatrix(vec3(1, -2, .5), -0.4));

	SDFScene scene = SDFScene({ tor, floor});


	Path templateShadersDir = Path("SDF\\templateShaders");
	Path shadersDir = Path("SDF\\shaders");
	Path generatedShadersDir = Path("SDF\\generatedShaders");
