...
namespace testsHelpers {
	inline vec4 multH(vec4 p, vec4 q) {
		return vec4(p.x * q.x - p.y * q.y - p.z * q.z - p.w * q.w, p.x * q.y + p.y * q.x + p.z * q.w - p.w * q.z, p.x * q.z - p.y * q.w + p.z * q.x + p.w * q.y, p.x * q.w + p.y * q.z - p.z * q.y + p.w * q.x);
	}
	...
	inline mat3 doubleCoverSO3(vec4 q) {
		...
		return mat3(x0 * x0 + x1 * x1 - x2 * x2 - x3 * x3, 2.0 * (x1 * x2 - x0 * x3), 2.0 * (x1 * x3 + x0 * x2), 2.0 * (x1 * x2 + x0 * x3), x0 * x0 - x1 * x1 + x2 * x2 - x3 * x3, 2.0 * (x2 * x3 - x0 * x1), 2.0 * (x1 * x3 - x0 * x2), 2.0 * (x2 * x3 + x0 * x1), x0 * x0 - x1 * x1 - x2 * x2 + x3 * x3);
	}

	inline float normMat3(mat3 m) {
		return std::sqrt(m[0][0] * m[0][0] + m[0][1] * m[0][1] + m[0][2] * m[0][2] + m[1][0] * m[1][0] + m[1][1] * m[1][1] + m[1][2] * m[1][2] + m[2][0] * m[2][0] + m[2][1] * m[2][1] + m[2][2] * m[2][2]);
	}
	...
	inline vec4 randomUnitQuaternion() {
		auto v = vec4(randomFloat(-1, 1), randomFloat(-1, 1), randomFloat(-1, 1), randomFloat(-1, 1));
		return v / length(v);
	}

	inline vec3 randomVec3() {
		auto v = vec3(randomFloat(-1, 1), randomFloat(-1, 1), randomFloat(-1, 1));
		return v;
	}
	...
	inline SE3 multSE3(const SE3 &a, const SE3 &b) {
		return SE3(multH(a.quat, b.quat), a.v + rotateVectorByQuaternion(b.v, a.quat));
	}

	inline SE3 inv_SE3(const SE3 &a) {
		return SE3(invH(a.quat), rotateVectorByQuaternion(-a.v, conjH(a.quat)));
	}
...
inline bool doubleCoverTest() {
	bool passed = true;
	for (int _ = 0; _ < 10; _++) {
		vec4 q = testsHelpers::randomUnitQuaternion();
		mat3 m = testsHelpers::doubleCoverSO3(q);
...
inline bool SE3Test() {
	...
	for (int _ = 0; _ < 10; _++) {
		...
		passed &= assertTrue_UT(testsHelpers::nearlyEqualSE3( testsHelpers::multSE3(testsHelpers::multSE3(g1, g2), g3), testsHelpers::multSE3(g1, testsHelpers::multSE3(g2, g3)) ));
		auto a = testsHelpers::multSE3(testsHelpers::multSE3(g1, g2), g3);
		auto b = testsHelpers::multSE3(g1, testsHelpers::multSE3(g2, g3));

		passed &= assertTrue_UT(testsHelpers::nearlyEqualSE3( testsHelpers::multSE3(g1, testsHelpers::inv_SE3(g1)), testsHelpers::id_SE3() ));


		passed &= assertNearlyEqual_UT(testsHelpers::transformPoint(g1_inv, testsHelpers::transformPoint(g1, p)), p);
	}
	return passed;
}
...
inline UnitTestResult quatGLSLModuleTests__all() {
	...
	result.runTest(multTests);
	result.runTest(doubleCoverTest);
	result.runTest(rotationsQuatTest);
	result.runTest(SE3Test);
