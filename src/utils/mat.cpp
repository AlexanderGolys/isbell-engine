...
vec5 operator+(const vec5 &a, const vec5 &b) { return vec5(a.x + b.x, a.y + b.y, a.z + b.z, a.w + b.w, a.v + b.v);
}
...
Complex Complex::operator+(float c) const {return (*this)+Complex(c);}
...
Complex Complex::operator+(int c) const {return (*this)+Complex(c);}
...
Complex operator*(int f, const Complex &c) {return 1.f*f*c;}
...
Quaternion Quaternion::operator*(Quaternion r) const {
	return Quaternion(
		q.x * r.q.x - q.y * r.q.y - q.z * r.q.z - q.w * r.q.w,
		q.x * r.q.y + q.y * r.q.x + q.z * r.q.w - q.w * r.q.z,
		q.x * r.q.z - q.y * r.q.w + q.z * r.q.x + q.w * r.q.y,
		q.x * r.q.w + q.y * r.q.z - q.z * r.q.y + q.w * r.q.x);
}
...
void FloatVector::append(const vector<float> &v) { for (float f: v) this->data.push_back(f); }
vector<float> FloatVector::getVec() const { return this->data; }
...
vector<float> FloatMatrix::operator[](int i) { return this->data[i]; }
...
FloatMatrix::FloatMatrix(vector<vector<float>> &&data) { this->data = std::move(data); }
