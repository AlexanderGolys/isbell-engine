...
int BufferManager::addFullVertexData(const Vertex3DSmart& v) {
...
BufferedVertex::BufferedVertex(BufferManager& bufferBoss, const Vertex3DSmart& v)
...
Vertex3DSmart BufferedVertex::getVertex() const {
	...
	return Vertex3DSmart(getPosition(), getUV(), getNormal(), getColor(), extraData);
}
...
void BufferedVertex::setVertex(const Vertex3DSmart& v) {
...
Vertex3DSmart IndexedTriangle::getVertex(int i) const {
...
IndexedMesh::IndexedMesh(const vector<Vertex3DSmart>& hardVertices, const vector<ivec3>& faceIndices, const PolyGroupID& id)
: IndexedMesh() {
...
void IndexedMesh::addNewPolygroup(const char* filename, const PolyGroupID& id) {
	...
	while (getline(in, line)) {
		if (line.substr(0, 2) == "v ") {
			...
			vertices[index].emplace_back(*boss, Vertex3DSmart(vert, UV_DEFAULT, NORM_DEFAULT));
		}
		...
		if (line.substr(0, 3) == "vn ") {
			...
			for (int i = 0; i < 3; i++)
				if (!vertexIndicesFilled.contains(vertexIndex[i])) {
					...
					vertices[index].emplace_back(*boss, Vertex3DSmart(pos[vertexIndex[i]], uvsVec[i], normsVec[i]));
...
void IndexedMesh::addNewPolygroup(const vector<Vertex3DSmart>& hardVertices, const vector<ivec3>& faceIndices, const PolyGroupID& id) {
	if (polygroupIndexOrder.contains(id))
		throw IllegalVariantError("Polygroup ID already exists in mesh. ", __FILE__, __LINE__);
	...
	for (const Vertex3DSmart& v : hardVertices)
...
void IndexedMesh::addUniformSurface(const SmoothParametricSurface& surf, int tRes, int uRes, const PolyGroupID& id) {
	vector<Vertex3DSmart> hardVertices = {};
...
void IndexedMesh::copyPolygroup(const IndexedMesh& other, const PolyGroupID& id, const PolyGroupID& newId) {
	vector<Vertex3DSmart> vertices = other.getVertices(id);
...
void IndexedMesh::copyPolygroup(const PolyGroupID& id, const PolyGroupID& newId) {
	vector<Vertex3DSmart> vertices = getVertices(id);
...
vector<Vertex3DSmart> IndexedMesh::getVertices(const PolyGroupID& id) const {
...
IndexedMesh IndexedMesh::subdivideBarycentric(const PolyGroupID& id) const {
	...
	vector<Vertex3DSmart> verts = getVertices(id);
	verts.reserve(verts.size() + trInds.size());
	vector<ivec3> newInds = {};
	newInds.reserve(3 * trInds.size());
	for (ivec3 tr : trInds) {
		Vertex3DSmart b = barycenter(verts[tr.x], verts[tr.y], verts[tr.z]);
		verts.push_back(b);
...
IndexedMesh IndexedMesh::wireframe(PolyGroupID id, PolyGroupID targetId, float width, float heightCenter, float heightSide) const {
	...
	vector<Vertex3DSmart> new_verts = getVertices(id);
	vector<ivec3> new_inds = {};
	for (const auto& v : verts) {
		Vertex3DSmart extr = v.getVertex();
...
Vertex3DSmart BufferManager::getVertex(int index) const {
	dict(string, vec4) extraData = {};
	if (hasExtra0())
	...
	return Vertex3DSmart(getPosition(index), getUV(index), getNormal(index), getColor(index), extraData);
}
...
: id(id), settings(r, horRes, radialRes) {
	...
	vector<Vertex3DSmart> verts = vector<Vertex3DSmart>();
...
SurfacePlotDiscretisedMesh::SurfacePlotDiscretisedMesh(const DiscreteRealFunctionR2& plot) {
	vector<Vertex3DSmart> points = vector<Vertex3DSmart>();
