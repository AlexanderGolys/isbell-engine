...
Vertex::Vertex(vec3 position, vec2 uv, vec3 normal, vec4 color, std::map<string, vec4> extraData) {
...
Vertex::Vertex(vec3 position, vec3 normal, vec4 color, std::map<string, vec4> extraData) {
	this->position = position;
	this->normal = normal;
...
bool Vertex::hasIndex() const {
	return index != -1;
}

void Vertex::gotAddedAsVertex(std::weak_ptr<IndexedTriangle> triangle, int i) {
	triangles.push_back({triangle, i});
}

vector<std::pair<std::weak_ptr<IndexedTriangle>, int>> Vertex::getTriangles() const {
	return triangles;
}
...
vec3 Vertex::getExtraData_xyz(const string& name) const {
	return vec3(extraData.at(name));
}

float Vertex::getExtraData(const string& name, int i) const {
	return extraData.at(name)[i];
}

float Vertex::getExtraLast(const string& name) const {
	return extraData.at(name)[3];
}

bool Vertex::hasExtraData() const {
...
vector<string> Vertex::getExtraDataNames() const {
	return keys(extraData);
}
...
Vertex Vertex::operator+(vec3 v) const {
	return Vertex(this->position + v, this->uv, this->normal, this->color, this->extraData);
}

void Vertex::operator+=(vec3 v) {
	this->translate(v);
}
...
void Vertex::setAllParametricCurveExtras(float t, const CurveSample& sample) {
	setCurveParameter(t);
...
Vertex barycenter(Vertex v1, const Vertex& v2, const Vertex& v3) {
	...
	return Vertex(barycenter(v1.getPosition(), v2.getPosition(), v3.getPosition()), barycenter(v1.getUV(), v2.getUV(), v3.getUV()), n,
...
Vertex center(Vertex v1, const Vertex& v2) {
	...
	return Vertex((v1.getPosition() + v2.getPosition()) / 2.0f, (v1.getUV() + v2.getUV()) / 2.0f, normalize(v1.getNormal() + v2.getNormal()),
