...
TriangleR3::TriangleR3(const vector<vec3>& vertices, const vector<vec3>& normals)
: TriangleR3() {
	this->vertices = vertices;
...
TriangleR3::TriangleR3(const vector<vec3>& vertices, const vector<vec4>& colors)
: TriangleR3(vertices) {
	this->vertexColors = colors;
}

TriangleR3::TriangleR3(const vector<vec3>& vertices, vec3 normal)
: TriangleR3(vertices) {
	this->normals = {normal, normal, normal};
}

TriangleR3::TriangleR3(const vector<vec3>& vertices, const vector<vec2>& uvs)
: TriangleR3(vertices, vec3(0, 0, 1)) {
	this->uvs = uvs;
}

TriangleR3::TriangleR3(const vector<vec3>& vertices, vec4 color)
: TriangleR3(vertices, vec3(0, 0, 1)) {
	this->vertexColors = {color, color, color};
}


TriangleR3::TriangleR3(const vector<vec3>& vertices, const vector<vec3>& normals, vec4 color)
: TriangleR3(vertices, normals) {
	this->vertexColors = {color, color, color};
}

TriangleR3::TriangleR3(const vector<vec3>& vertices, vec3 normal, vec4 color)
: TriangleR3(vertices, normal) {
	this->vertexColors = {color, color, color};
}

TriangleR3::TriangleR3(const vector<vec3>& vertices, const vector<vec3>& normals, const vector<vec4>& colors)
: TriangleR3(vertices, normals) {
	this->vertexColors = colors;
}

TriangleR3::TriangleR3(const vector<vec3>& vertices, vec3 normal, const vector<vec4>& colors)
: TriangleR3(vertices, normal) {
	this->vertexColors = colors;
}


TriangleR3::TriangleR3(const vector<vec3>& vertices, const vector<vec3>& normals, vec4 color, const vector<vec2>& uvs)
: TriangleR3(vertices, normals, color) {
	this->uvs = uvs;
}


TriangleR3::TriangleR3(const vector<vec3>& vertices, vec3 normal, vec4 color, const vector<vec2>& uvs)
: TriangleR3(vertices, normal, color) {
	this->uvs = uvs;
}

TriangleR3::TriangleR3(const vector<vec3>& vertices, const vector<vec3>& normals, const vector<vec4>& colors, const vector<vec2>& uvs, MaterialPhong material)
: TriangleR3(vertices, normals, colors, uvs) {
	this->material = material;
}

TriangleR3::TriangleR3(vector<vec3> vertices, vector<vec3> normal, MaterialPhong material) {}

TriangleR3::TriangleR3(vector<vec3> vertices, MaterialPhong material) {}

bool TriangleR3::hasMaterial() const {
	return material.has_value();
}

void TriangleR3::addMaterial(MaterialPhong material) {
	this->material = material;
}

TriangleR3::TriangleR3(const vector<vec3>& vertices, const vector<vec3>& normals, const vector<vec4>& colors, const vector<vec2>& uvs)
: TriangleR3(vertices, normals, colors) {
	this->uvs = uvs;
}

TriangleR3::TriangleR3(const vector<vec3>& vertices, vec3 normal, const vector<vec4>& colors, const vector<vec2>& uvs)
: TriangleR3(vertices, normal, colors) {
	this->uvs = uvs;
}
...
Vertex3DSmart::Vertex3DSmart(vec3 position, vec2 uv, vec3 normal, vec4 color, std::map<string, vec4> extraData, std::optional<MaterialPhongConstColor> material) {
...
Vertex3DSmart::Vertex3DSmart(vec3 position, vec3 normal, vec4 color, std::map<string, vec4> extraData, std::optional<MaterialPhongConstColor> material) {
	this->position = position;
	this->normal = normal;
...
string Vertex3DSmart::hash() const {
	return to_string(this->position.x) + to_string(this->position.y) + to_string(this->position.z) + to_string(this->normal.x) + to_string(this->normal.y) +
		to_string(this->normal.z) + to_string(this->uv.x) + to_string(this->uv.y) + to_string(this->color.x) + to_string(this->color.y) + to_string(this->color.z) +
		to_string(this->color.w);
}

bool Vertex3DSmart::operator<(const Vertex3DSmart& v) const {
	if (this->position.x != v.position.x)
		return this->position.x < v.position.x;
	if (this->position.y != v.position.y)
		return this->position.y < v.position.y;
	if (this->position.z != v.position.z)
		return this->position.z < v.position.z;
	if (this->normal.x != v.normal.x)
		return this->normal.x < v.normal.x;
	if (this->normal.y != v.normal.y)
		return this->normal.y < v.normal.y;
	if (this->normal.z != v.normal.z)
		return this->normal.z < v.normal.z;
	if (this->uv.x != v.uv.x)
		return this->uv.x < v.uv.x;
	if (this->uv.y != v.uv.y)
		return this->uv.y < v.uv.y;
	if (this->color.x != v.color.x)
		return this->color.x < v.color.x;
	if (this->color.y != v.color.y)
		return this->color.y < v.color.y;
	if (this->color.z != v.color.z)
		return this->color.z < v.color.z;
...
void Vertex3DSmart::setIndex(int i) {
	index = i;
}

int Vertex3DSmart::getIndex() const {
	return index;
}

bool Vertex3DSmart::hasIndex() const {
	return index != -1;
}

void Vertex3DSmart::gotAddedAsVertex(std::weak_ptr<IndexedTriangle> triangle, int i) {
	triangles.push_back({triangle, i});
}

vector<std::pair<std::weak_ptr<IndexedTriangle>, int>> Vertex3DSmart::getTriangles() const {
	return triangles;
}

vec3 Vertex3DSmart::getPosition() const {
	return position;
}

vec3 Vertex3DSmart::getNormal() const {
	return normal;
}

vec2 Vertex3DSmart::getUV() const {
	return uv;
}

vec4 Vertex3DSmart::getColor() const {
	return color;
}

MaterialPhong Vertex3DSmart::getMaterial() const {
	return material.value();
}

mat4 Vertex3DSmart::getMaterialMat() const {
	return material.value().compressToMatrix();
}

vec4 Vertex3DSmart::getExtraData(const string& name) const {
	return extraData.at(name);
}

vec3 Vertex3DSmart::getExtraData_xyz(const string& name) const {
	return vec3(extraData.at(name));
}

float Vertex3DSmart::getExtraData(const string& name, int i) const {
	return extraData.at(name)[i];
}

float Vertex3DSmart::getExtraLast(const string& name) const {
	return extraData.at(name)[3];
}

bool Vertex3DSmart::hasExtraData() const {
	return !extraData.empty();
}

bool Vertex3DSmart::hasExtraData(const string& name) const {
	return extraData.contains(name);
}

void Vertex3DSmart::addExtraData(const string& name, vec4 data) {
...
void Vertex3DSmart::addExtraData(const string& name, vec3 data) {
	if (extraData.contains(name)) {
...
void Vertex3DSmart::addExtraData(const string& name, float data, int i) {
	if (!extraData.contains(name))
		extraData.insert({name, vec4(0)});
	extraData[name][i] = data;
}

void Vertex3DSmart::translate(vec3 v) {
	this->position += v;
}

vector<string> Vertex3DSmart::getExtraDataNames() {
	vector<string> names;
...
bool Vertex3DSmart::hasMaterial() const {
	return material.has_value();
}

void Vertex3DSmart::transform(const SpaceEndomorphism& M) {
	this->normal = M.df(this->position) * this->normal;
	this->position = M(this->position);
}


Vertex3DSmart Vertex3DSmart::operator+(vec3 v) const {
	return Vertex3DSmart(this->position + v, this->uv, this->normal, this->color, this->extraData, this->material);
}

void Vertex3DSmart::operator+=(vec3 v) {
	this->translate(v);
}

void Vertex3DSmart::setMaterial(MaterialPhongConstColor material) {
	this->material = material;
}

void Vertex3DSmart::setPosition(vec3 position) {
	this->position = position;
}

void Vertex3DSmart::setNormal(vec3 normal) {
	this->normal = normal;
}

void Vertex3DSmart::setUV(vec2 uv) {
	this->uv = uv;
}

void Vertex3DSmart::setColor(vec4 color) {
	this->color = color;
}
...
void Vertex3DSmart::appendToBuffers(StdAttributeBuffers& buffers) {
...
void Vertex3DSmart::appendExtraDataToBuffer(const string& name, vector<vec4>& buffer) const {
	if (extraData.contains(name))
		buffer.push_back(extraData.at(name));
}

void Vertex3DSmart::appendToList(vector<Vertex3DSmart>& list) {
	setIndex(list.size());
	list.push_back(*this);
}

void Vertex3DSmart::setAllParametricCurveExtras(float t, CurveSample& sample) {
	setCurveParameter(t);
...
Vertex3DSmart barycenter(Vertex3DSmart v1, const Vertex3DSmart& v2, const Vertex3DSmart& v3) {
	...
	return Vertex3DSmart(barycenter(v1.getPosition(), v2.getPosition(), v3.getPosition()), barycenter(v1.getUV(), v2.getUV(), v3.getUV()), n,
...
Vertex3DSmart center(Vertex3DSmart v1, const Vertex3DSmart& v2) {
	std::map<string, vec4> extraData = {};
	for (auto& key : v1.getExtraDataNames()) {
	...
	if (v1.hasMaterial() && v2.hasMaterial() && !v1.getMaterial().textured() && !v2.getMaterial().textured())
		return Vertex3DSmart((v1.getPosition() + v2.getPosition()) / 2.0f, (v1.getUV() + v2.getUV()) / 2.0f, normalize(v1.getNormal() + v2.getNormal()),
							 (v1.getColor() + v2.getColor()) / 2.0f, extraData, MaterialPhongConstColor((v1.getMaterialMat() + v2.getMaterialMat()) / 2.0f));
	return Vertex3DSmart((v1.getPosition() + v2.getPosition()) / 2.0f, (v1.getUV() + v2.getUV()) / 2.0f, normalize(v1.getNormal() + v2.getNormal()),
...
TriangularMesh::TriangularMesh(const vector<TriangleR3>& triangles)
: TriangularMesh() {
...
TriangularMesh::TriangularMesh(const char* filename, MeshFormat format)
: TriangularMesh() {
	if (format == 0) {
		FILE* file = fopen(filename, "r");
		if (file == NULL) {
...
void TriangularMesh::randomizeFaceColors(vec3 min, vec3 max) {
	...
	for (int i = 0; i < triangles.size(); i++) {
		vec4 faceColor = vec4(distribution(generator) * (max.x - min.x) + min.x, distribution(generator) * (max.y - min.y) + min.y,
...
void TriangularMesh::applyMap(const std::function<vec3(vec3)>& f, const std::function<vec3(vec3, vec3)>& f_normal) {
	for (int i = 0; i < this->triangles.size(); i++) {
		for (int j = 0; j < 3; j++) {
			triangles[i].vertices[j] = f(triangles[i].vertices[j]);
...
void TriangularMesh::applyMap(std::function<vec3(vec3)> f, std::function<vec3(vec3, vec3)> f_normal, string customDomain) {}


MaterialPhong::MaterialPhong()
: MaterialPhong(vec4(0), vec4(0), vec4(0), 0, 0, 0, 0) {}
...
MaterialPhong::MaterialPhong(const shared_ptr<Texture>& texture_ambient, const shared_ptr<Texture>& texture_diffuse, const shared_ptr<Texture>& texture_specular,
...
MaterialPhong::MaterialPhong(const string& textureFilenameAmbient, const string& textureFilenameDiffuse, const string& textureFilenameSpecular, float ambientIntensity,
							 float diffuseIntensity, float specularIntensity, float shininess, bool alphaAmbient, bool alphaDiffuse, bool alphaSpecular)
: MaterialPhong(makeAmbientTexture(textureFilenameAmbient, alphaAmbient), makeDiffuseTexture(textureFilenameDiffuse, alphaDiffuse),
				makeSpecularTexture(textureFilenameSpecular, alphaSpecular), ambientIntensity, diffuseIntensity, specularIntensity, shininess) {}
...
void MaterialPhong::addDiffuseTexture(const string& filename, bool alpha) {
	setDiffuseTexture(makeDiffuseTexture(filename, alpha));
}

void MaterialPhong::addSpecularTexture(const string& filename, bool alpha) {
	setSpecularTexture(makeSpecularTexture(filename, alpha));
}

void MaterialPhong::addAmbientTexture(vec4 color) {
	setAmbientTexture(constAmbientTexture(color));
}

void MaterialPhong::addDiffuseTexture(vec4 color) {
	setDiffuseTexture(constDiffuseTexture(color));
}

void MaterialPhong::addSpecularTexture(vec4 color) {
	setSpecularTexture(constSpecularTexture(color));
}

MaterialPhongConstColor::MaterialPhongConstColor(vec4 ambient, vec4 diffuse, vec4 specular, float ambientIntensity, float diffuseIntensity, float specularIntensity,
												 float shininess)
: MaterialPhong(ambient, diffuse, specular, ambientIntensity, diffuseIntensity, specularIntensity, shininess), ambientColor(ambient), diffuseColor(diffuse),
  specularColor(specular) {}

MaterialPhongConstColor::MaterialPhongConstColor(mat4 mat)
: MaterialPhongConstColor(mat[0], mat[1], mat[2], mat[3][0], mat[3][1], mat[3][2], mat[3][3]) {}

MaterialPhongConstColor::MaterialPhongConstColor()
: MaterialPhongConstColor(vec4(0), vec4(0), vec4(0), 0, 0, 0, 0) {}

mat4 MaterialPhongConstColor::compressToMatrix() const {
	return mat4(ambientColor, diffuseColor, specularColor, compressIntencities());
}

bool MaterialPhongConstColor::textured() const {
	return false;
}
...
TriangleComplex::TriangleComplex(const array<Complex, 3>& vertices)
: TriangleComplex(vertices, {vec2(vertices[0]), vec2(vertices[1]), vec2(vertices[2])}) {}

TriangleComplex::operator TriangleR2() const {
	vector<vec2> vertices = {
		vec2(this->vertices[0].x(), this->vertices[0].y()), vec2(this->vertices[1].x(), this->vertices[1].y()), vec2(this->vertices[2].x(), this->vertices[2].y())
	};
...
array<TriangleComplex, 3> TriangleComplex::subdivide() const {
	...
	return {
		TriangleComplex({vertices[0], vertices[1], center}, {vertexColors[0], vertexColors[1], centerColor}, {uvs[0], uvs[1], centerUV}),
...
Vertex3DSmart::Vertex3DSmart(const Vertex3DSmart& other)
: position(other.position), normal(other.normal), uv(other.uv), color(other.color), material(other.material), extraData(other.extraData), index(other.index),
  triangles(other.triangles) {}

Vertex3DSmart::Vertex3DSmart(Vertex3DSmart&& other) noexcept
: position(std::move(other.position)), normal(std::move(other.normal)), uv(std::move(other.uv)), color(std::move(other.color)), material(std::move(other.material)),
  extraData(std::move(other.extraData)), index(other.index), triangles(std::move(other.triangles)) {}

Vertex3DSmart& Vertex3DSmart::operator=(const Vertex3DSmart& other) {
	if (this == &other)
		return *this;
...
Vertex3DSmart& Vertex3DSmart::operator=(Vertex3DSmart&& other) noexcept {
	if (this == &other)
		return *this;
...
MeshFamily1P::MeshFamily1P(const shared_ptr<TriangularMesh>& mesh, const std::function<std::function<vec3(vec3)>(float, float)>& time_operator,
...
MeshFamily1PExtraDomain::MeshFamily1PExtraDomain(const shared_ptr<TriangularMesh>& embedded_mesh, const string& domain,
												 const std::function<PlaneAutomorphism(float)>& time_operator_from0, vec3 embedding_shift)
: MeshFamily1P(embedded_mesh, time_operator_from0, embedding_shift) {
	this->domain = domain;
}
...
SuperMesh::SuperMesh(TriangularMesh& mesh, const MaterialPhongConstColor& material)
: SuperMesh(mesh) {
...
SuperMesh::SuperMesh(const char* filename, const MaterialPhongConstColor& material, MeshFormat format)
: SuperMesh(filename, format) {
...
void SuperMesh::precomputeBuffers(bool materials, bool extra) {
	if (this->stdAttributeBuffers.positions.size() == 0) {
		...
		this->bufferLocations = {
			&this->stdAttributeBuffers.positions[0], &this->stdAttributeBuffers.normals[0], &this->stdAttributeBuffers.colors[0], &this->stdAttributeBuffers.uvs[0]
		};

		this->bufferSizes = {
			(int)this->stdAttributeBuffers.positions.size(), (int)this->stdAttributeBuffers.normals.size(), (int)this->stdAttributeBuffers.colors.size(),
			(int)this->stdAttributeBuffers.uvs.size()
		};
	}
	...
	if (materials && this->materialBuffers.ambientColors.size() == 0) {
		...
		this->bufferLocations = {
			&this->stdAttributeBuffers.positions[0], &this->stdAttributeBuffers.normals[0], &this->stdAttributeBuffers.colors[0], &this->stdAttributeBuffers.uvs[0],
			&this->materialBuffers.ambientColors[0], &this->materialBuffers.diffuseColors[0], &this->materialBuffers.specularColors[0],
			&this->materialBuffers.intencitiesAndShininess[0]
		};

		this->bufferSizes = {
			(int)this->stdAttributeBuffers.positions.size(), (int)this->stdAttributeBuffers.normals.size(), (int)this->stdAttributeBuffers.colors.size(),
			(int)this->stdAttributeBuffers.uvs.size(), (int)this->materialBuffers.ambientColors.size(), (int)this->materialBuffers.diffuseColors.size(),
...
SuperCurve::SuperCurve(const SmoothParametricCurve& curve, const std::function<float(float)>& width, const std::function<MaterialPhongConstColor(float)>& material, int segments,
...
vector<CurveSample> sampleCurve(SmoothParametricCurve curve, std::function<float(float)> width, std::function<MaterialPhongConstColor(float)> material, float t0, float t1, int n,
								bool periodic) {
...
: position(other.position), normal(other.normal), tangent(other.tangent), material(other.material), width(other.width), extraInfo(other.extraInfo) {}

CurveSample::CurveSample(CurveSample&& other) noexcept
: position(std::move(other.position)), normal(std::move(other.normal)), tangent(std::move(other.tangent)), material(std::move(other.material)), width(other.width),
...
vector<CurveSample> sampleCurve(SmoothParametricCurve curve, float width, MaterialPhongConstColor material, float t0, float t1, int n, bool periodic) {
	return sampleCurve(curve, [w=width](float t) {
						   return w;
					   }, [m=material](float t) {
						   return m;
					   }, t0, t1, n, periodic);
...
void SuperPencilCurve::addLocalDeformation(HOM(float, SpaceEndomorphism) _local_operator, float t) {
	this->_ambient_operator = make_unique<BIHOM(float, float, SpaceEndomorphism)>([op=_local_operator](float t1, float t2) {
		return op(t2 - t1);
	});
	this->_t = t;
}
