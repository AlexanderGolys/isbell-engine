...
inline string plural(string word) {
	...
	bool es = word.back() == 's' || word.back() == 'x' || word.back() == 'z' || (word.size() > 1 && word[word.size() - 2] == 'c' && word.back() == 'h') || (word.size() > 1 && word[
		word.size() - 2] == 's' && word.back() == 'h') || word.back() == 'S' || word.back() == 'X' || word.back() == 'Z' || (word.size() > 1 && word[word.size() - 2] == 'C' && word
		.back() == 'H') || (word.size() > 1 && word[word.size() - 2] == 'S' && word.back() == 'H');
	if (es && !caps)
		return word + "es";
	if (es)
		return word + "ES";
...
public:
	KeyError(const string& key, const string& containerName, const char* file, int line)
	: IndexError("Key " + key + " not found in " + containerName + ".", file, line) {}

	KeyError(const string& msg, const char* file, int line)
	: IndexError(msg, file, line) {}
};
...
public:
	IteratorError(const string& msg, const char* file, int line)
	: IndexError(msg, file, line) {}

	IteratorError(const char* file, int line)
	: IndexError("Iterator operation error.", file, line) {}
};

class IteratorEndReferenceError : public IteratorError {
public:
	IteratorEndReferenceError(const string& msg, const char* file, int line)
	: IteratorError(msg, file, line) {}

	IteratorEndReferenceError(const char* file, int line)
	: IteratorError("Reference to terminal iterator undefined", file, line) {}
};

class IteratorEndIncrementError : public IteratorError {
public:
	IteratorEndIncrementError(const string& msg, const char* file, int line)
	: IteratorError(msg, file, line) {}

	IteratorEndIncrementError(const char* file, int line)
	: IteratorError("Incrementing terminal iterator undefined", file, line) {}
};
