...
public:
	...
	static void createProgram(gl_id& shaderID);
	...
	static void createShader(gl_id& shaderID, GLenum shaderType);
	...
	static void createVAO(gl_id& voaID);

	static void createTexture(gl_id& id);
	static void deleteTexture(gl_id& id);
	static void bindTexture2D(gl_id id, uint slot);
	...
	static void loadTexture2(GLenum internalFormat, array_len width, array_len height, GLenum format, raw_data_ptr<uchar> data);
	static void setSampler2D(const string& samplerName, uint slot, gl_id programID);
	...
	static void initUBO(gl_id bufferID, int bindingPoint, const string& blockName, gl_id currentProgram);
...
public:
	...
	static GLFWwindow* createWindow(int width, int height, string_cr title);
	static void destroyWindow(GLFWwindow* window);
	static void pollEvents();
	static void swapFrameBuffers(GLFWwindow* window);

	static void setWindowData(GLFWwindow* window, raw_data_mut data);
	static WindowSettings& getWindowData(GLFWwindow* window);
	static vec2 getCursorPosition(GLFWwindow* window);
	static bool isWindowOpen(GLFWwindow* window);
};