...
public:
	...
	MaterialPhong(const shared_ptr<Texture>& texture_ambient, const shared_ptr<Texture>& texture_diffuse, const shared_ptr<Texture>& texture_specular, float ambientIntensity,
				  float diffuseIntensity, float specularIntensity, float shininess);


	MaterialPhong(const string& textureFilenameAmbient, const string& textureFilenameDiffuse, const string& textureFilenameSpecular, float ambientIntensity, float diffuseIntensity,
				  float specularIntensity, float shininess, bool alphaAmbient = false, bool alphaDiffuse = false, bool alphaSpecular = false);
...
class Vertex3DSmart {
public:
	Vertex3DSmart(const Vertex3DSmart& other);
	Vertex3DSmart(Vertex3DSmart&& other) noexcept;
	Vertex3DSmart& operator=(const Vertex3DSmart& other);
	Vertex3DSmart& operator=(Vertex3DSmart&& other) noexcept;
...
public:
	Vertex3DSmart(vec3 position, vec2 uv, vec3 normal = e3, vec4 color = BLACK, std::map<string, vec4> extraData = {}, optional<MaterialPhongConstColor> material = std::nullopt);
	explicit Vertex3DSmart(vec3 position, vec3 normal = e3, vec4 color = BLACK, std::map<string, vec4> extraData = {}, optional<MaterialPhongConstColor> material = std::nullopt);
	...
	string hash() const;
	bool operator<(const Vertex3DSmart& v) const;
	bool operator==(const Vertex3DSmart& v) const;
	void setIndex(int i);
	int getIndex() const;
	bool hasIndex() const;
	...
	vector<weak_ptr<Vertex3DSmart>> getNeighbours() const;
	...
	Vertex3DSmart operator+(vec3 v) const;
	...
	void appendToList(vector<Vertex3DSmart>& list);
	void addTriangle(shared_ptr<IndexedTriangle> triangle, int i);

	void setCurveParameter(float t) {
		addExtraData("curvePoint", t);
	}

	void setCurvePosition(vec3 pos) {
		addExtraData("curvePoint", pos);
	}

	void setCurveTangent(vec3 v) {
		addExtraData("curveTangent", v);
	}

	void setCurveNormal(vec3 v) {
		addExtraData("curveNormal", v);
	}

	void setCurveNormalAngle(float a) {
		addExtraData("curveTangent", a);
	}

	void setAllParametricCurveExtras(float t, CurveSample& sample);

	float getCurveParameter() {
		return getExtraLast("curvePoint");
	}

	vec3 getCurvePosition() {
		return getExtraData_xyz("curvePoint");
	}

	vec3 getCurveTangent() {
		return getExtraData_xyz("curveTangent");
	}

	vec3 getCurveNormal() {
		return getExtraData_xyz("curveNormal");
	}

	float getCurveNormalAngle() {
		return getExtraLast("curveTangent");
	}

	float getCurveWidth() {
		return norm(getPosition() - getCurvePosition());
	}
};
...
public:
	Light(vec3 position, vec4 color, vec4 intensities, float mode, float r)
	: position(position), color(color), intensities(intensities), mode(mode), softShadowRadius(r) {}

	virtual ~Light() = default;

	virtual mat4 compressToMatrix() const {
		return mat4(vec4(position, 0), color, intensities, vec4(mode, softShadowRadius, 0, 0));
	}
...
public:
	PointLightQuadric(vec3 position, vec4 color, float intensity, float r = .1)
	: Light(position, color, vec4(intensity, 0, 0, 0), 0, r), intensity(intensity) {}

	PointLightQuadric()
	: PointLightQuadric(vec3(0, 0, 0), vec4(1, 1, 1, 1), 1) {}
...
public:
	...
	TriangularMesh(vector<Vertex3DSmart> vertices, vector<ivec3> faceIndices);
...
public:
	...
	MeshFamily1P(const shared_ptr<TriangularMesh>& mesh, const std::function<std::function<vec3(vec3)>(float, float)>& time_operator,
				 const std::function<std::function<vec3(vec3, vec3)>(float, float)>& time_operator_normal, float t = 0);
	MeshFamily1P(const shared_ptr<TriangularMesh>& embedded_mesh, std::function<PlaneSmoothEndomorphism(float, float)> time_operator, vec3 embedding_shift = vec3(0, 0, 0),
				 float t = 0);
...
public:
	MeshFamily1PExtraDomain(const shared_ptr<TriangularMesh>& mesh, const string& domain, const std::function<std::function<vec3(vec3)>(float, float)>& time_operator,
							const std::function<std::function<vec3(vec3, vec3)>(float, float)>& time_operator_normal, float t = 0);
	MeshFamily1PExtraDomain(const shared_ptr<TriangularMesh>& embedded_mesh, const string& domain, const std::function<PlaneSmoothEndomorphism(float, float)>& time_operator,
							vec3 embedding_shift = vec3(0, 0, 0), float t = 0);
	MeshFamily1PExtraDomain(const shared_ptr<TriangularMesh>& embedded_mesh, const string& domain, const std::function<PlaneAutomorphism(float)>& time_operator_from0,
							vec3 embedding_shift = vec3(0, 0, 0));
...
class SuperPencilPlanar : public SuperMesh {
	float _t = 0;
	std::function<Biholomorphism(float)> _time_operator = [](float t) {
		return Biholomorphism::linear(1 + 0i, 0i);
	};
...
public:
	CurveSample(vec3 position, vec3 normal, vec3 tangent, MaterialPhongConstColor material, float width)
	: position(position), normal(normal), tangent(tangent), material(material.compressToMatrix()), width(width) {}
	...
	MaterialPhongConstColor getMaterial() const {
		return MaterialPhongConstColor(material);
	}

	mat4 getMaterialMatrix() const {
		return material;
	}

	float readExtra(int i) const {
		return extraInfo[i];
	}

	float readExtraLast() const {
		return extraInfo.w;
	}

	vec4 readExtra() const {
		return extraInfo;
	}

	vec3 readExtraXYZ() const {
		return vec3(extraInfo);
	}

	void updatePosition(vec3 new_position) {
		position = new_position;
	}

	void updateNormal(vec3 new_normal) {
		normal = new_normal;
	}

	void updateTangent(vec3 new_tangent) {
		tangent = new_tangent;
	}

	void updateMaterial(const MaterialPhongConstColor& new_material) {
		material = new_material.compressToMatrix();
	}

	void updateWidth(float new_width) {
		width = new_width;
	}

	void updateExtra(vec4 extra) {
		extraInfo = extra;
	}

	void updateExtra(vec3 extra) {
		extraInfo = vec4(extra, extraInfo.w);
	}

	void updateExtra(float extra, int position = 3) {
		extraInfo[position] = extra;
	}
};

vector<CurveSample> sampleCurve(SmoothParametricCurve curve, std::function<float(float)> width, std::function<MaterialPhongConstColor(float)> material, float t0 = 0, float t1 = 1,
								int n = 100, bool periodic = false);


vector<CurveSample> sampleCurve(SmoothParametricCurve curve, float width, MaterialPhongConstColor material, float t0 = 0, float t1 = 1, int n = 100, bool periodic = false);
...
const std::map<CurveEmbeddingTypeID, string> curveEmbeddingTypes = {{PLANAR, "PLANAR"}, {TUBE, "TUBE"}, {NOT_EMBEDDED, "NOT EMBEDDED"}};
...
class SuperCurve {
	...
	void initMesh() {
		_mesh = std::make_shared<SuperMesh>();
	}

public:
	SuperCurve(const SmoothParametricCurve& curve, const std::function<float(float)>& width, const std::function<MaterialPhongConstColor(float)>& material, int segments,
			   float t0 = 0, float t1 = 1, bool periodic = true);
...
public:
	...
	SuperPencilCurve(const SuperCurve& other)
	: SuperCurve(other) {}

	SuperPencilCurve(SuperCurve&& other) noexcept
	: SuperCurve(std::move(other)) {}
