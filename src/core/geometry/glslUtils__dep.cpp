...
int lengthOfGLSLType(ShaderDataType type) {
	...
	std::cout << "Error: unknown ShaderDataType" << std::endl;
	return -1;
}
...
void ShaderProgram::initUniforms(const std::unordered_map<string, ShaderDataType> &uniforms) {
...
void ShaderProgram::setUniform(const string &uniformName, const GLfloat *uniformValue) {
	ShaderDataType uniformType = this->uniformTypes[uniformName];
...
: Camera(std::make_shared<SmoothParametricCurve>([speed, radius, height](float t){
...
Camera::Camera(const shared_ptr<SmoothParametricCurve> &trajectory, const shared_ptr<SmoothParametricCurve> &lookAtPos, vec3 upVector, float fov_x, float aspectRatio, float clippingRangeMin, float clippingRangeMax) {
...
Camera::Camera(const shared_ptr<SmoothParametricCurve> &trajectory, const shared_ptr<SmoothParametricCurve> &lookAtPos, const std::function<vec3(float)> &upVector, float fov_x, float aspectRatio, float clippingRangeMin, float clippingRangeMax) {
	this->lookAtFunc = lookAtPos;
	this->up = upVector;
	this->fov_x = fov_x;
	this->aspectRatio = aspectRatio;
...
Attribute::Attribute(const string &name, ShaderDataType type, int inputNumber, CommonBufferType bufferType) {
	this->name = name;
	this->type = type;
...
void Attribute::load(const void *firstElementAdress, int bufferLength) {
	if (!bufferInitialized) {
		...
		glBufferData(GL_ARRAY_BUFFER, bufferLength * size, firstElementAdress, GL_STATIC_DRAW);
		return;
	}
	glBindBuffer(GL_ARRAY_BUFFER, this->bufferAddress);
	glBufferData(GL_ARRAY_BUFFER, bufferLength * size, firstElementAdress, GL_STATIC_DRAW);
...
RenderingStep::RenderingStep(const shared_ptr<ShaderProgram> &shader) {
	...
	this->uniforms = std::map<string, ShaderDataType>();
...
void RenderingStep::addUniform(string uniformName, ShaderDataType uniformType, shared_ptr<std::function<void(float, shared_ptr<ShaderProgram>)> > setter) {
...
void RenderingStep::addConstFloats(const std::map<string, float> &uniforms) {
	for (auto uniform: uniforms)
		addUniform(uniform.first, FLOAT, std::make_shared<std::function<void(float, shared_ptr<ShaderProgram>)> >([uniform](float t, const shared_ptr<ShaderProgram> &shader){
...
void RenderingStep::addUniforms(const std::map<string, ShaderDataType> &uniforms, std::map<string, shared_ptr<std::function<void(float, shared_ptr<ShaderProgram>)> > > setters) {
...
: bgColor(bgColor), alphaBlending(alphaBlending), depthTest(depthTest), timeUniform(timeUniform), speed(speed), maxFPS(maxFPS), takeScreenshots(takeScreenshots), resolution(resolution), screenshotFrequency(screenshotFrequency), windowTitle(windowTitle) {
	if (takeScreenshots)
...
void Renderer::addPerFrameUniforms(const std::map<string, ShaderDataType> &uniforms, const std::map<string, shared_ptr<std::function<void(float, shared_ptr<ShaderProgram>)> > > &setters) {
	for (const auto &renderingStep: renderingSteps)
		renderingStep->addUniforms(uniforms, setters);
}

void Renderer::addPerFrameUniform(const string &uniformName, ShaderDataType uniformType, const shared_ptr<std::function<void(float, shared_ptr<ShaderProgram>)> > &setter) {
	for (const auto &renderingStep: renderingSteps)
...
void Renderer::addConstUniform(const string &uniformName, ShaderDataType uniformType, shared_ptr<std::function<void(shared_ptr<ShaderProgram>)> > setter) {
...
void Renderer::addTimeUniform() {
	addPerFrameUniform("time", FLOAT, std::make_shared<std::function<void(float, shared_ptr<ShaderProgram>)> >([](float t, const shared_ptr<ShaderProgram> &shader){
...
void Renderer::addConstUniforms(const std::map<string, ShaderDataType> &uniforms, std::map<string, shared_ptr<std::function<void(shared_ptr<ShaderProgram>)> > > setters) {
	for (const auto &uniform: uniforms)
