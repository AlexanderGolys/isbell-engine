...
int BufferManager::bufferLength(CommonBufferType type) const {
	...
	switch (type) {
		...
		case EXTRA2:
		case EXTRA3:
		case EXTRA4: return stds->positions.size();
		case INDEX: return indices->size();
	}
...
void *BufferManager::firstElementAddress(CommonBufferType type) const {
	switch (type) {
		case POSITION: return &stds->positions[0];
		case NORMAL: return &stds->normals[0];
		case UV: return &stds->uvs[0];
		case COLOR: return &stds->colors[0];
		case EXTRA0: return &(*extra0)[0];
		case EXTRA1: return &(*extra1)[0];
		case EXTRA2: return &(*extra2)[0];
		case EXTRA3: return &(*extra3)[0];
		case EXTRA4: return &(*extra4)[0];
		case INDEX: return &(*indices)[0];
	}
	throw UnknownVariantError("Buffer not recognised among common types. ", __FILE__, __LINE__);
}
...
bool BufferManager::hasMaterial() const {
	return isActive(MATERIAL1);
}
...
IndexedTriangle::IndexedTriangle(BufferManager &bufferBoss, ivec3 index, int shift) : index(bufferBoss.addTriangleVertexIndices(index, shift)), bufferBoss(bufferBoss) {}
...
IndexedMesh::IndexedMesh(IndexedMesh &&other) noexcept : boss(std::move(other.boss)), vertices(std::move(other.vertices)), triangles(std::move(other.triangles)), polygroupIndexOrder(std::move(other.polygroupIndexOrder)) {}
...
void IndexedMesh::addNewPolygroup(const vector<Vertex> &hardVertices, const vector<ivec3> &faceIndices, const PolyGroupID &id) {
	THROW_IF(polygroupIndexOrder.contains(id), IllegalVariantError, "Polygroup ID already exists in mesh. ");
...
IndexedMesh::IndexedMesh(const IndexedMesh &other) : boss(&*other.boss), vertices(other.vertices), polygroupIndexOrder(other.polygroupIndexOrder), triangles(other.triangles) {}
...
BufferManager::BufferManager(const BufferManager &other) : stds(std::make_unique<Stds>(*other.stds)), extra0(std::make_unique<vector<vec4> >(*other.extra0)), extra1(std::make_unique<BUFF4 >(*other.extra1)), extra2(std::make_unique<BUFF4 >(*other.extra2)), extra3(std::make_unique<BUFF4 >(*other.extra3)), extra4(std::make_unique<BUFF4 >(*other.extra4)), indices(std::make_unique<IBUFF3 >(*other.indices)), extraBufferNames(other.extraBufferNames), activeBuffers(other.activeBuffers) {}
...
BufferManager::BufferManager(BufferManager &&other) noexcept : stds(std::move(other.stds)), extra0(std::move(other.extra0)), extra1(std::move(other.extra1)), extra2(std::move(other.extra2)), extra3(std::move(other.extra3)), extra4(std::move(other.extra4)), indices(std::move(other.indices)), activeBuffers(std::move(other.activeBuffers)), extraBufferNames(std::move(other.extraBufferNames)) {}
...
vec4 BufferManager::getExtra(int index, int slot) const {
	switch (slot) {
		case 0: return (*extra0)[index];
		case 1: return (*extra1)[index];
		case 2: return (*extra2)[index];
		case 3: return (*extra3)[index];
		case 4: return (*extra4)[index];
		default: ;
	}
...
void BufferManager::setExtra(int index, vec4 value, int slot) {
	switch (slot) {
		case 0: (*extra0)[index] = value;
			return;
		case 1: (*extra1)[index] = value;
			return;
		case 2: (*extra2)[index] = value;
			return;
		case 3: (*extra3)[index] = value;
			return;
		case 4: (*extra4)[index] = value;
			return;
	}
	throw UnknownVariantError("Extra slot not recognised. ", __FILE__, __LINE__);
}
...
void BufferManager::setExtra(int index, float value, int slot, int component) {
	switch (slot) {
		case 0: (*extra0)[index][component] = value;
			return;
		case 1: (*extra1)[index][component] = value;
			return;
		case 2: (*extra2)[index][component] = value;
			return;
		case 3: (*extra3)[index][component] = value;
			return;
		case 4: (*extra4)[index][component] = value;
			return;
		default: throw UnknownVariantError("Extra slot not recognised. ", __FILE__, __LINE__);
	}
}
...
PlanarDiffusedPatterns::PlanarDiffusedPatterns(const VectorFieldR2 &X, float dt, int steps, const vector<vec4> &colors, const vector<vec2> &shifts, const HOM(float, float) &time_dump, const HOM(float, float) &width_ratio_dump, const SmoothParametricPlaneCurve &curve_pattern, vec2 domain, int resolution, float t0) {
...
PipeCurveVertexShader::PipeCurveVertexShader(const SmoothParametricCurve &curve, float r, int horRes, int radialRes, const PolyGroupID &id, dict(string, vec4) extra_defaults) : PipeCurveVertexShader(curve, PIPE_SETTINGS(r, horRes, radialRes, extra_defaults), id) {}
...
void PipeCurveVertexShader::updateCurve(const SmoothParametricCurve &curve) {
	deformPerVertex(id, [this, &curve](BufferedVertex &v){
		float t = v.getUV().x;
		vec3 p = curve(t);
		p = vec3(min(max(p.x, settings.bound_min.x), settings.bound_max.x), min(max(p.y, settings.bound_min.y), settings.bound_max.y), min(max(p.z, settings.bound_min.z), settings.bound_max.z));
		vec3 b = curve.binormal(t);
		vec3 n = curve.normal(t);
...
mat3 IndexedMesh::inertiaTensorAppBd(PolyGroupID id, vec3 p) const {
	...
	return integrateOverTriangles<mat3>([ cm](const IndexedTriangle &t){
		vec3 p = t.center();
		return mat3(pow((p - cm).z, 2) + pow((p - cm).y, 2), -(p - cm).x * (p - cm).y, -(p - cm).x * (p - cm).z, -(p - cm).x * (p - cm).y, pow((p - cm).x, 2) + pow((p - cm).z, 2), -(p - cm).y * (p - cm).z, -(p - cm).x * (p - cm).z, -(p - cm).y * (p - cm).z, pow((p - cm).x, 2) + pow((p - cm).y, 2));
	}, id);
}
...
: IndexedMesh(other), foliation(other.foliation), special_leaves(other.special_leaves), continuous_leaves(other.continuous_leaves), leaf_radial_res(other.leaf_radial_res), leaf_hor_res(other.leaf_hor_res), color_map(other.color_map), special_leaf_colors(other.special_leaf_colors), leaf_radius_map(other.leaf_radius_map), special_leaf_radii(other.special_leaf_radii), id_prefix(other.id_prefix) {
	// boss = make_unique<BufferManager>(std::set({POSITION, NORMAL, UV, COLOR, INDEX, EXTRA0}));
}

FoliatedParametricSurfaceMesh::FoliatedParametricSurfaceMesh(FoliatedParametricSurfaceMesh &&other) noexcept
: IndexedMesh(std::move(other)), foliation(std::move(other.foliation)), special_leaves(other.special_leaves), continuous_leaves(other.continuous_leaves), leaf_radial_res(other.leaf_radial_res), leaf_hor_res(other.leaf_hor_res), color_map(std::move(other.color_map)), special_leaf_colors(std::move(other.special_leaf_colors)), leaf_radius_map(std::move(other.leaf_radius_map)), special_leaf_radii(std::move(other.special_leaf_radii)), id_prefix(std::move(other.id_prefix)) {
...
FoliatedParametricSurfaceMesh::FoliatedParametricSurfaceMesh(ParametricSurfaceFoliation foliation, int special_leaves, int continuous_leaves, int leaf_radial_res, int leaf_hor_res, std::function<vec4(float)> color_map, const vector<vec4> &special_leaf_colors, HOM(float, float) leaf_radius_map, const vector<float> &special_leaf_radii, std::function<vec4(float)> extra1_map, const vector<vec4> &special_extra1s)
: IndexedMesh(), foliation(foliation), special_leaves(special_leaves), continuous_leaves(continuous_leaves), leaf_radial_res(leaf_radial_res), leaf_hor_res(leaf_hor_res), color_map(color_map), special_leaf_colors(special_leaf_colors), leaf_radius_map(leaf_radius_map), special_leaf_radii(special_leaf_radii), extra1_map(extra1_map), special_extra1s(special_extra1s) {
