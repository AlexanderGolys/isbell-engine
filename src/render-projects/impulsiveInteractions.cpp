...
WindowSettings windowSettings = WindowSettings(TypicalResolution::HD2K, "window", false, false);
RenderSettings settings = RenderSettings(windowSettings, Color(0.05f,0.05f,0.05), true, true, true, true, 2.f, 1.0f);
CameraSettings cameraSettings = CameraSettings();

Path shaderDir = ConfigFile::getProjectRoot() / "src/core/shaders/shaders";
...
int main() {
	...
	0.06f, 0.4f, 1.9f, 12.0f);

	auto materialModel2 = make_shared<MaterialModelPhong>(
	Color(0.8f, 0.2f, 0.2f),
	Color(0.95f, 0.2f, 0.2f),
	Color(1, 0.9f, 0.9f),
	0.07f, 0.6f, 1.7f, 42.0f);


	auto surf1 = SmoothParametricSurface([](float u, float v) {
		float R = 4.5;
		float r = (1.5 + sin(6*u)/8 )*(1+sin(u)/3);
		return vec3((R + r * cos(v)) * cos(u), (R + r * cos(v)) * sin(u), -r * sin(v));
	}, vec2(0, TAU), vec2(0, TAU), true, true, .001f);

	auto ellips = ellipsoid(2, 1, 5.3f, vec3(0, 0, 2), .001f);
	auto surf2 = SmoothParametricSurface([ellips](float u, float v) {
		vec3 p = ellips(u, v);
		return p + vec3((1.f+p.z)*cos(2*p.z)/3, 0, 0);
	}, ellips.get_rangeT(), ellips.get_rangeU(), ellips.get_periodicT(), ellips.get_periodicU(), ellips.get_epsilon());

	auto mesh1 = make_shared<Mesh3D>(surf1, 1501, 1501);
	auto mesh2 = make_shared<Mesh3D>(surf2, 1500, 1500);



	auto light1 = SimplePointLight(vec3(5, -12, 14), WHITE, vec3(1, 0.01f, 0.001f));
	auto light2 = SimplePointLight(vec3(-15, 5, 15), WHITE, vec3(1, 0.01f, 0.001f));
	...
	auto camera = make_shared<Camera>(cameraSettings, vec3(16, 16, 17), vec3(0, 0, 0), vec3(0, 0, 1));
	...
	auto dist2 = *Distribution::inverseGamma(3, .6, 6);

	auto camShiftHor = make_shared<CameraHorisontalShiftOnKey>(camera, dist2, GLFW_KEY_RIGHT, GLFW_KEY_LEFT, 0.1f);
	auto camShiftVer = make_shared<CameraVerticalShiftOnKey>(camera, dist, GLFW_KEY_UP, GLFW_KEY_DOWN, 0.1f);
	auto camRotationHor = make_shared<CameraHorisontalRotationOnKey>(camera, dist2, GLFW_KEY_D, GLFW_KEY_A, 0.1f);
	...
	scene->addGlobalComponent(camShiftHor);
	scene->addGlobalComponent(camShiftVer);
	scene->addGlobalComponent(camRotationHor);
	scene->addGlobalComponent(camRotationVer);
	scene->addGlobalComponent(camZoom);
	scene->addGlobalComponent(camRotate);
