...
int main() {
	...
	auto f = [](float t) { return (t*t)/(t*t-1)*(t*t-4)/(t*t-6); };
	...
	auto floor = make_shared<WeakSuperMesh>(flatsurf, 100, 300);
	auto floor2 = make_shared<WeakSuperMesh>(projsurf, 100, 300);



	auto proj = make_shared<WeakSuperMesh>(PipeCurveVertexShader(projcurve, pipe_settings_comp));

	auto flat = make_shared<WeakSuperMesh>(PipeCurveVertexShader(flatcurve, pipe_settings));


	auto infaxis = make_shared<WeakSuperMesh>(PipeCurveVertexShader(infline, pipe_infaxis));
