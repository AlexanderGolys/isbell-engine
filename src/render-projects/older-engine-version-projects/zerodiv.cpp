...
int main() {
	...
	auto floor = make_shared<IndexedMesh3D>(flatsurf, 100, 300);
	auto floor_proj = make_shared<IndexedMesh3D>(projsurf(0), 100, 300);

	auto proj = make_shared<PipeCurveVertexShader>(projcurve(0), pipe_settings_comp);
	auto flat = make_shared<IndexedMesh3D>(PipeCurveVertexShader(flatcurve, pipe_settings));

	auto infaxis = make_shared<IndexedMesh3D>(PipeCurveVertexShader(infline, pipe_infaxis));
