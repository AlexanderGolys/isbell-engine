...
protected:
	...
	std::vector<FloatVector> cellAttributes;
	BIHOM(int, float, FloatVector) cellAttributeChange;
...
public:
	...
	FloatVector attributesWithNbhrs(int i) const;
	FloatVector meanVertexAttributes(int i) const;
...
class FluidSimulationFVM : public FluidSimulation {
	std::vector<FloatMatrix> attributeMatrices;
public:
	FluidSimulationFVM(const std::vector<ivec6> &hexahedra, const std::vector<glm::ivec4> &faces,
		const std::vector<Vertex> &vertices, const std::vector<ivec2> &faceNbhd,
		const std::vector<int>& bdFaces,
		const MATR$X& initialAttributes, std::vector<FloatMatrix> attributeMatrices,
		BIHOM(BufferedVertex&, std::vector<float>&, void) updateBdWithCellIndex);
	...
	FluidSimulationFVM(HexVolumetricMeshWithBoundary mesh,
		HOM(HexahedronWithNbhd&, FloatVector) initAttr, HOM(HexahedronWithNbhd&, FloatMatrix) initMatrices,
		BIHOM(BufferedVertex&, std::vector<float>&, void) updateBdWithCellIndex);
...
public:
	explicit IncompressibleNewtonianFluid2D(const vector<vector<IncompressibleCell>> &cells)
	:  cells(flatten(cells)), gridSize(cells.size(), cells[0].size()), M(6*gridSize.x*gridSize.y, 6*gridSize.x*gridSize.y) {}
	void updateMatrix (float dt);
	vec69 velocities_x_vector() const;
	vec69 velocities_y_vector() const;
	void updatePressuresGradients(const vec69 &pressures_x, const vec69 &pressures_y);
	...
	void updateDensities(const vec69 &densities) {
		for (int i = 0; i < cells.size(); i++)
			cells[i].updateDensity(densities[i]);
	}
